...
namespace M3D
{
	...
	Quaternion Quaternion::LookAt(Vector3 sourcePoint, Vector3 destPoint)
	{
		...
		float d = dot(Vector3::FORWARD, forwardVector);

		if (abs(d - (-1.0f)) < 0.000001f)
		{
			return Quaternion(3.1415926535897932f, Vector3::UP.x, Vector3::UP.y, Vector3::UP.z);
		}
		if (abs(d - (1.0f)) < 0.000001f)
		{
		...
		float rotAngle = (float)acos(d);
		Vector3 rotAxis = cross(Vector3::FORWARD, forwardVector);
	...
	Quaternion Quaternion::fromToRotation(const Vector3& fromDirection, const Vector3& toDirection)
	{
		...
		else if (d <= -1.0f)
		{
			...
			return angleAxis(3.14159265359f, axis.normalized());
		}
		else
		{
			// Scalar component.
